...
u16  SendMessage(u8* Psend, unsigned short iSize, u8* pDeviceID, u16 Address_Node)
{
   ...
   //mput_mix_sx1278((char*)(Psend), iSize);
	 ...
	 Delay_ms(10000);                                  //为了方便发送完配置成功帧以后接收服务器会话结束命令，在此处增加延时
...
/*******************************************************************************
* Function Name  : void  SX1287_Init(u16 sNodeID,u8 NetWorkID)
* Description    : 初始化433模块
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void  SX1287_Init(u16 sNodeID,u8 NetWorkID)
{
  u16  UartConfig   =0;
	u16  CentralFreq  =0;
	u16  NodeID       =0;
  ...
  u16  Temp =0;	
	...
	SetNodeNetworkID (NetWorkID);                      //设置433模块网络ID：0
